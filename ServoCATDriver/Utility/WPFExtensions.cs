    ...
    public static class WPFExtensions {
        ...
        public static Rect GetAbsolutePosition(this Window w) {
            ...
            if (!multimonSupported) {
                ...
                return new Rect(rc.left / scalingFactor, rc.top / scalingFactor, rc.width / scalingFactor, rc.height / scalingFactor);
            } else {
                var helper = new WindowInteropHelper(w);
                var hmonitor = OSInterop.MonitorFromWindow(new HandleRef((object)null, helper.EnsureHandle()), 2);
                var info = new OSInterop.MONITORINFOEX();
                OSInterop.GetMonitorInfo(new HandleRef((object)null, hmonitor), info);
                return new Rect(info.rcWork.left / scalingFactor, info.rcWork.top / scalingFactor, info.rcWork.width / scalingFactor, info.rcWork.height / scalingFactor);
            }
        }
    }
}